  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-6 text-center">
        ...
        <p className="text-3xl font-bold text-darkBlue">{formatCurrency(results.annualSavings)}</p>
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-4 text-center">
          ...
          <p className="text-2xl font-bold text-darkBlue">
            {typeof results.paybackPeriod === 'number' ? `${formatYears(results.paybackPeriod)} ` : 'N/A'}
          </p>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-4 text-center">
          ...
          <p className="text-2xl font-bold text-darkBlue">{results.annualIRR !== null ? `${formatPercentage(results.annualIRR)}` : 'N/A'}</p>
          <p className="text-xs text-gray-500 mt-1">Calculated over 3 years</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-indigo-100 rounded-xl p-4 text-center">
        ...
        <p className="text-2xl font-bold text-darkBlue">{typeof results.roiPercentage3Year === 'number' ? `${formatPercentage(results.roiPercentage3Year)}` : 'N/A'}</p>
        <p className="text-xs text-gray-500 mt-1">Calculated over 3 years</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-lightGreen">{formatCurrency(results.laborSavings)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-darkBlue">Material Savings:</span>
            <span className="font-bold text-lightGreen">{formatCurrency(results.materialSavings)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-darkBlue">Material Waste Savings:</span>
            <span className="font-bold text-lightGreen">{formatCurrency(results.materialWasteSavings)}</span>
          </div>
